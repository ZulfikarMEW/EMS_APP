...
export class LazyForDirective implements DoCheck, OnInit {
  ...
  private update(): void {
    ...
    const fixedHeaderHeight =
      ...
      (this.lazyForContainer.getBoundingClientRect().top -
        this.lazyForContainer.scrollTop);

    //This needs to run after the scrollTop is retrieved.
    this.vcr.clear();

    let listStartI = Math.floor(
      (scrollTop - fixedHeaderHeight) / this.itemHeight
    );
    listStartI = this.limitToRange(listStartI, 0, this.list.length);

    let listEndI = Math.ceil(
      (scrollTop - fixedHeaderHeight + listHeight) / this.itemHeight
    );
    ...
    this.afterListElem.style.height = `${
      (this.list.length - listEndI - 1) * this.itemHeight
    }px`;
  }
  ...
  private onFirstUpdate(): void {
    ...
    // If you want to use <tr> elements
    if (this.itemTagName.toLowerCase() === "tr") {
      this.beforeListElem.style.listStyleType = "none";
      this.afterListElem.style.listStyleType = "none";
    }
