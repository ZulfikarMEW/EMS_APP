...
import { PersonalPriorExperience } from "./PersonalPriorExperience";
import { VPersonalPriorExperience } from "./VPersonalPriorExperience";

@Component({
  selector: "app-previos-experience",
  templateUrl: "./previos-experience.component.html",
  styleUrls: ["./previos-experience.component.scss"],
})
export class PreviosExperienceComponent implements OnInit {
  ...
  dataList: VPersonalPriorExperience[] = [];
  historyDataList: VPersonalPriorExperience[] = [];
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      ...
      { label: "Prior Experience", active: true },
    ];
    ...
    this.form = this.fb.group({
      ...
      MasterWorkPlaceId: ["", [Validators.required]],
  ...
  fetchData() {
    ...
    this.personalService
      .postGetPersonalExperience(this.subDataParams)
      .subscribe(
        (x) => {
  ...
  fetchMasterData(opt) {
    ...
    // }
    if (opt != undefined) {
      if (opt == 18 && this.masterData.workPlaceList.length <= 2) {
        this.loadMasterList(opt);
      } else if (opt == 8 && this.masterData.jobLevelList.length <= 2) {
        this.loadMasterList(opt);
      }
      //   } else if (
      //     opt == 9 &&
  ...
  loadMasterList(opt) {
    ...
    if (opt == 18) {
      this.masterData.workPlaceList = [];
      this.masterService.getMasterDataList(18, null).subscribe(
        (x) => {
          Object.assign(this.masterData.workPlaceList, x);
          this.isLoading = false;
        },
        (err) => {
          Swal.fire("Failed!", "Something went wrong.", "error");
      ...
      this.masterData.jobLevelList = [];
      this.masterService.getMasterDataList(8, null).subscribe(
        (x) => {
          Object.assign(this.masterData.jobLevelList, x);
          this.isLoading = false;
        },
        (err) => {
          Swal.fire("Failed!", "Something went wrong.", "error");
  ...
  removeHistory(item: VPersonalPriorExperience) {
    Swal.fire({
      title: "Are you sure?",
      ...
      if (result.value) {
        this.personalService.postRemovePersonalExperience(item).subscribe(
          (x) => {
            console.log(x);
            Swal.fire(
              "Saved!",
              "Personal Experience data removed successfully.",
              "success"
            );
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          let a = new PersonalPriorExperience();

          a.Id = this.form.value.Id;
          a.PersonalDataId = this.subDataParams.Id; //this.form.value.PersonalDataId;
          a.MasterWorkPlaceId = this.form.value.MasterWorkPlaceId;
          a.MasterJobLevelId = this.form.value.MasterJobLevelId;
          a.DateFrom = this.form.value.DateFrom;
          a.DateTo = this.form.value.DateTo;
          a.Note = this.form.value.Note;

          this.personalService.postSavePersonalExperience(a).subscribe(
            (x) => {
              console.log(x);
              Swal.fire(
                "Saved!",
                "Personal Experience data saved successfully.",
                "success"
              );
