...
import { MasterQualification } from "../../masters/qualification/MasterQualification";
import { PersonalDataParams } from "../personal-data/personalDataParams";
import { PersonalData } from "../PersonalData";
import { PersonalQualification } from "./PersonalQualification";
import { VPersonalQualification } from "./VPersonalQualification";
...
export class AddqualificationComponent implements OnInit {
  ...
  dataList: VPersonalQualification[] = [];
  historyDataList: VPersonalQualification[] = [];

  masterData: MasterQualification[] = [];
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      ...
      { label: "Qualification Management", active: true },
    ];
    ...
    this.form = this.fb.group({
      ...
      MasterQualificationId: ["", [Validators.required]],
      DateQualified: ["", [Validators.required]],
      Note: [""],
    });
  }
  ...
  fetchData() {
    ...
    this.personalService
      .postGetPersonalQualification(this.subDataParams)
      .subscribe(
        (x) => {
  ...
  removeHistory(item: VPersonalQualification) {
    Swal.fire({
      title: "Are you sure?",
      ...
      if (result.value) {
        this.personalService.postRemovePersonalQualification(item).subscribe(
          (x) => {
            console.log(x);
            Swal.fire(
              "Saved!",
              "Personal Qualification data removed successfully.",
              "success"
            );
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          let a = new PersonalQualification();
          ...
          this.personalService.postSavePersonalQualification(a).subscribe(
            (x) => {
              console.log(x);
              Swal.fire(
                "Saved!",
                "Personal Qualification data saved successfully.",
                "success"
              );
