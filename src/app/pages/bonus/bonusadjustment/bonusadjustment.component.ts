...
import { VBonusEligibleList } from "../manualentry/VBonusEligibleList";
import { VBonusList } from "./VBonusList";

@Component({
  selector: "app-bonusadjustment",
  templateUrl: "./bonusadjustment.component.html",
  styleUrls: ["./bonusadjustment.component.scss"],
})
export class BonusadjustmentComponent implements OnInit {
  ...
  fetchHistoryData(id) {
    if (!this.yearForm.invalid) {
      ...
      this.bonusService.postGetBonusList(id).subscribe(
        (x) => {
          Object.assign(this.historyDataList, x);
  ...
  openModal(dataModal: any, item: VBonusEligibleList) {
    this.form.reset();

    // this.subDataParams = item;

    this.form.controls.PersonalDataId.setValue(item.Id);
  ...
  openHistoryModal(historyModal: any, item: VBonusEligibleList) {
    this.form.reset();
    // this.subDataParams = item;
    this.fetchHistoryData(item.Id);
    //console.log(this.dataList);
    // this.personalDataParams = new PersonalDataParams();
    //this.modalService.open(dataModal);
  ...
  removeHistory(item: Bonus) {
    Swal.fire({
      ...
      if (result.value) {
        this.bonusService.postRemoveBonusAdjustment(item).subscribe(
          (x) => {
            //console.log(x);
            Swal.fire("Saved!", "Bonus data removed successfully.", "success");
  ...
  onSubmit() {
    ...
    this.form.controls.Year.setValue(this.yearForm.value.Year);
    ...
    if (this.form.valid) {
      ...
      Swal.fire({
        title: "Are you sure?",
        text: "You want to save bonus adjustment?",
        icon: "question",
        showCancelButton: true,
        confirmButtonColor: "#34c38f",
        cancelButtonColor: "#f46a6a",
        ...
        if (result.value) {
          ...
          this.bonusService.postSaveAdjustment(this.form.value).subscribe(
            (x) => {
              //console.log(x);
              Swal.fire(
                "Saved!",
                "Bonus adjustment saved successfully.",
                "success"
              );
