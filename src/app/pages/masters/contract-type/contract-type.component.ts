...
import { MasterContractType } from "./MasterContractType";
...
export class ContractTypeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Masters" },
      { label: "Contract Type", active: true },
    ];
  ...
  fetchData() {
    this.isLoading = true;
    this.masterService.getMasterDataList(7, null).subscribe(
      (x) => {
  ...
  openEditModal(dataModal: any, item: MasterContractType) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(7, a).subscribe(
        (res) => {
          console.log(res);
