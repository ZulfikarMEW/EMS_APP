...
import { MasterReasonForRetirement } from "./MasterReasonForRetirement";

@Component({
  selector: "app-reason-for-retirement",
  templateUrl: "./reason-for-retirement.component.html",
  styleUrls: ["./reason-for-retirement.component.scss"],
})
export class ReasonForRetirementComponent implements OnInit {
  breadCrumbItems: Array<{}>;
  event: EventEmitter<any> = new EventEmitter();

  masterDataList: MasterData[] = [];
  dataList: MasterReasonForRetirement[] = [];
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Masters" },
      { label: "Reason For Retirement", active: true },
    ];
  ...
  fetchData() {
    this.isLoading = true;
    this.masterService.getMasterDataList(17, null).subscribe(
      (x) => {
  ...
  openEditModal(dataModal: any, item: MasterReasonForRetirement) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(17, a).subscribe(
        (res) => {
          console.log(res);
