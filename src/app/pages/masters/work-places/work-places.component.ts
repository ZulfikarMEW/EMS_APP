...
import { MasterWorkPlace } from "./MasterWorkPlace";
...
export class WorkPlacesComponent implements OnInit {
  ...
  openEditModal(dataModal: any, item: MasterWorkPlace) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(18, a).subscribe(
        (res) => {
          console.log(res);
