...
import { MasterReasonForPromotion } from "./MasterReasonForPromotion";

@Component({
  selector: "app-reason-for-promotion",
  templateUrl: "./reason-for-promotion.component.html",
  styleUrls: ["./reason-for-promotion.component.scss"],
})
export class ReasonForPromotionComponent implements OnInit {
  breadCrumbItems: Array<{}>;
  event: EventEmitter<any> = new EventEmitter();

  masterDataList: MasterData[] = [];
  dataList: MasterReasonForPromotion[] = [];
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Masters" },
      { label: "Reason For Promotion", active: true },
    ];
  ...
  openEditModal(dataModal: any, item: MasterReasonForPromotion) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(21, a).subscribe(
        (res) => {
          console.log(res);
