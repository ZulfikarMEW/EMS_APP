...
import { MasterJobDescription } from "./MasterJobDescription";
...
export class JobDescriptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Masters" },
      { label: "Job Description", active: true },
    ];
  ...
  fetchData() {
    this.isLoading = true;
    this.masterService.getMasterDataList(9, null).subscribe(
      (x) => {
  ...
  openEditModal(dataModal: any, item: MasterJobDescription) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(9, a).subscribe(
        (res) => {
          console.log(res);
