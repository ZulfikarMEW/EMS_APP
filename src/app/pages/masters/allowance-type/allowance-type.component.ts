...
import { MasterAllowanceType } from "./MasterAllowanceType";

@Component({
  selector: "app-allowance-type",
  templateUrl: "./allowance-type.component.html",
  styleUrls: ["./allowance-type.component.scss"],
})
export class AllowanceTypeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Masters" },
      { label: "Allowance Type", active: true },
    ];
  ...
  fetchData() {
    this.isLoading = true;
    this.masterService.getMasterDataList(15, null).subscribe(
      (x) => {
  ...
  openEditModal(dataModal: any, item: MasterAllowanceType) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(15, a).subscribe(
        (res) => {
          console.log(res);
