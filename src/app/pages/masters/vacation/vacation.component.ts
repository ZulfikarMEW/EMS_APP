...
import { MasterVacationType } from "./MasterVacationType";
...
export class VacationComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Masters" },
      { label: "Vacation Type", active: true },
    ];
  ...
  fetchData() {
    this.isLoading = true;
    this.masterService.getMasterDataList(14, null).subscribe(
      (x) => {
  ...
  openEditModal(dataModal: any, item: MasterVacationType) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(14, a).subscribe(
        (res) => {
          console.log(res);
