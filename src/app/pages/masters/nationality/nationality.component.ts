...
export class NationalityComponent implements OnInit {
  ...
  fetchData() {
    this.isLoading = true;
    this.masterService.getMasterDataList(16, null).subscribe(
      (x) => {
  ...
  openEditModal(dataModal: any, item: MasterNationality) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(16, a).subscribe(
        (res) => {
          console.log(res);
