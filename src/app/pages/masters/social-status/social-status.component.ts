...
import { MasterSocialStatus } from "./MasterSocialStatus";

@Component({
  selector: "app-social-status",
  templateUrl: "./social-status.component.html",
  styleUrls: ["./social-status.component.scss"],
})
export class SocialStatusComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Masters" },
      { label: "Social Status", active: true },
    ];
  ...
  fetchData() {
    this.isLoading = true;
    this.masterService.getMasterDataList(19, null).subscribe(
      (x) => {
  ...
  openEditModal(dataModal: any, item: MasterSocialStatus) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(19, a).subscribe(
        (res) => {
          console.log(res);
