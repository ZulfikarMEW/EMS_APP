...
import { MasterJobLevel } from "./MasterJobLevel";
...
export class JobLevelComponent implements OnInit {
  ...
  openEditModal(dataModal: any, item: MasterJobLevel) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(8, a).subscribe(
        (res) => {
          console.log(res);
  ...
  onChangeFunctionalGroup(event: Event) {
