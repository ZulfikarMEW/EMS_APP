...
import { MasterDesignation } from "./MasterDesignation";
...
export class DesignationComponent implements OnInit {
  ...
  fetchData() {
    this.isLoading = true;
    this.masterService.getMasterDataList(6, null).subscribe(
      (x) => {
  ...
  openEditModal(dataModal: any, item: MasterDesignation) {
    this.form.reset();
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.masterService.postMasterData(6, a).subscribe(
        (res) => {
          console.log(res);
